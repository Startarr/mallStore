...
//    }
    ...
    public JsonResult<User> login(User user, HttpSession session) {
        /*
		  校验验证码：
		 */
        if(!stringRedisTemplate.hasKey(user.getVerificationCode())){
            return new JsonResult(JsonResult.CODE_ERR_BUSINESS, "验证码不正确！");
        }
        User data = userService.userLogin(user);
        //向session对象中完成数据的绑定(这个session是全局的,项目的任何位置都可以访问)

        if(data != null){
            CurrentUser currentUser = new CurrentUser(data.getUid(),data.getUsername(),data.getUsername());
            String token = tokenUtils.loginSign(currentUser, user.getPassword());
            Boolean delete = stringRedisTemplate.delete(user.getVerificationCode());
